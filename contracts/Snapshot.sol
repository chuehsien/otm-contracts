...
abstract contract Snapshot is Helper {
    ...
    modifier snapshotTrigger() {
        _dailySnapshotPoint(_currentOtmDay());
        _;
    }
        ...
        external
    {
        require(
            _updateDay > 0 &&
            _updateDay < _currentOtmDay()
            // 'OTM: snapshot day does not exist yet'
        );

        require(
            _updateDay > globals.currentOtmDay
            // 'OTM: snapshot already taken for that day'
        );
        ...
        private
    {
        ...
        for (uint256 _day = globals.currentOtmDay; _day < _updateDay; _day++) {
