...
contract LiquidityTransformer is usingProvable {
    ...
    uint256 constant YODAS_PER_OTM = 10 ** uint256(18);
    ...
    modifier afterInvestmentPhase() {
        require(
            _currentOtmDay() > INVESTMENT_DAYS,
            'OTM: ongoing investment phase'
        );
        _;
    }
    ...
    modifier investmentDaysRange(uint256 _investmentDay) {
        require(
            ...
            'OTM: not in initial investment days range'
        );
        _;
    }
    ...
    modifier onlyFundedDays(uint256 _investmentDay) {
        require(
            ...
            'OTM: no investments on that day'
        );
        _;
    }
    ...
    function defineToken(
        ...
        onlyTokenDefiner
    {
        OTM_CONTRACT = IOtmToken(_otmToken);
    ...
    constructor(address _otmToken, address _uniswapPair) {
    ...
    //  OTM RESERVATION (EXTERNAL FUNCTIONS)  //
    //  -------------------------------------  //

    /** @dev Performs reservation of OTM tokens with ETH
    ...
    /** @notice Allows reservation of OTM tokens with other ERC20 tokens
    ...
    //  OTM RESERVATION (INTERNAL FUNCTIONS)  //
    //  -------------------------------------  //
        ...
        internal
    {
        require(
            ...
            'OTM: must be a different address'
        );

        require(
            notContract(_referralAddress),
            'OTM: invalid referral address'
        );
    ...
    //  OTM RESERVATION (PRIVATE FUNCTIONS)  //
    //  ------------------------------------  //

    /** @notice Tracks investorTotalBalance and uniqueInvestors
      * @dev used in _reserveOtm() internal function
    ...
    {
        require(
            _investmentDay < _currentOtmDay(),
            'OTM: investment day must be in past'
        );

        require(
            g.generationDayBuffer == 0,
            'OTM: supply generation in progress'
        );

        require(
            dailyTotalSupply[_investmentDay] == 0,
            'OTM: supply already generated'
        );
    ...
    function _generateStaticSupply(
        ...
        internal
    {
        dailyTotalSupply[_investmentDay] = dailyMinSupply[_investmentDay] * YODAS_PER_OTM;
        ...
        override
    {
        require(
            ...
            'OTM: can only be called by Oracle'
        );

        require(
            g.generationDayBuffer > 0 &&
            g.generationDayBuffer <= INVESTMENT_DAYS,
            'OTM: incorrect generation day'
        );
        ...
        external
    {
        ...
        if (_currentOtmDay() - _investmentDay > 1) {
        ...
        afterInvestmentPhase
    {
        require(
            ...
            'OTM: incorrect referral batch'
        );

        require (
            g.preparedReferrals < referralAccountCount,
            'OTM: all referrals already prepared'
        );
    ...
    function _fullReferralBonus(address _referralAddress, uint256 _referralAmount, uint256 _ratio) internal {
        ...
        OTM_CONTRACT.giveStatus(_referralAddress);
    }
        ...
        afterInvestmentPhase
    {
        require(
            ...
            'OTM: must generate supply for all days'
        );

        require (
            g.preparedReferrals == referralAccountCount,
            'OTM: must prepare all referrals'
        );

        require (
            g.totalTransferTokens > 0,
            'OTM: must have tokens to transfer'
        );
        ...
        OTM_CONTRACT.mintSupply(
            address(this), _buffer
        );

        OTM_CONTRACT.approve(
    ...
    //  OTM TOKEN PAYOUT FUNCTIONS (INDIVIDUAL)  //
    //  ----------------------------------------  //
    ...
    /** @notice Allows to mint tokens for specific investor address
      ...
      * and uses OTM_CONTRACT instance to mint all the OTM tokens
    ...
    //  OTM TOKEN PAYOUT FUNCTIONS (BATCHES)  //
    //  -------------------------------------  //
        ...
        onlyFundedDays(_investmentDay)
    {
        require(
            ...
            'OTM: incorrect investment batch'
        );
    ...
    /** @notice checks that provided days are valid for investemnt
      * @dev used in reserveOtm() and reserveOtmWithToken()
      */
        ...
        pure
    {
        for (uint8 _i = 0; _i < _investmentDays.length; _i++) {
            require(
                _investmentDays[_i] >= _otmDay,
                'OTM: investment day already passed'
            );
            require(
                _investmentDays[_i] > 0 &&
                _investmentDays[_i] <= INVESTMENT_DAYS,
                'OTM: incorrect investment day'
            );
        }
    }

    /** @notice prepares path variable for uniswap to exchange tokens
      * @dev used in reserveOtmWithToken() swapExactTokensForETH call
    ...
    /** @notice OTM equivalent in ETH price calculation
    ...
    /** @notice shows current day of OtmToken
      * @dev value is fetched from OTM_CONTRACT
      * @return iteration day since OTM inception
      */
    function _currentOtmDay() public view returns (uint64) {
        return OTM_CONTRACT.currentOtmDay();
    }
    ...
    function requestRefund(
        ...
        require(
            ...
            _currentOtmDay() > INVESTMENT_DAYS + 10,
           unicode'OTM: liquidity successfully forwarded to uniswap 🦄'
        );
...
library SafeMathLT {
    ...
    function add(uint256 a, uint256 b) internal pure returns (uint256) {
        uint256 c = a + b;
        require(c >= a, 'OTM: addition overflow');
        return c;
    }

    function sub(uint256 a, uint256 b) internal pure returns (uint256) {
        require(b <= a, 'OTM: subtraction overflow');
        uint256 c = a - b;
        return c;
    }
    ...
    function mul(uint256 a, uint256 b) internal pure returns (uint256) {
        ...
        uint256 c = a * b;
        require(c / a == b, 'OTM: multiplication overflow');

        return c;
    }

    function div(uint256 a, uint256 b) internal pure returns (uint256) {
        require(b > 0, 'OTM: division by zero');
        uint256 c = a / b;
        return c;
    }

    function mod(uint256 a, uint256 b) internal pure returns (uint256) {
        require(b != 0, 'OTM: modulo by zero');
        return a % b;
    }
}
